...
class XmlToJsonCanonicalProcessorTests {
    ...
    @Test
    fun testCustomizedXmlReader() {
        val xml = """
            <h:thing xmlns:h="http://www.w3.org/TR/html4/">
        ...
        val parser = StreamingXmlParser(StringReader(xml), null)
        val field = parser.read()
        println(field)
    }
    ...
    fun testMyXmlReader() {
        ...
        val field = parser.parse(ByteArrayInputStream(xml.toByteArray()))
        val record = RecordCreator.create()
        record.set(Field.create(emptyMap<String, Field>()))
        ...
        val flatJson = writer.toString()
        println("Flat JSON")
        println(flatJson)
        println()
        println("Pretty JSON")
        val jsonTree = ObjectMapper().readTree(StringReader(flatJson))
